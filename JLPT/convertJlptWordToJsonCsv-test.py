...
def parse_question_start(line: str):
    line = norm(line)
    m = re.match(r"^([0-9]{1,3})\s*(?:[\.:：、\)）]|\s)\s*(.*)$", line)
    if m:
...
def split_by_expected_question(line: str, expected_qno: int):
    ...
    if expected_qno < 5:
        return [line]
    s = norm(line)
    pattern = re.compile(rf"(?<!\d){expected_qno}\s*[\.|、]\s*")
    m = pattern.search(s)
    if m and m.start() > 0:
...
def parse_questions(question_lines):
    ...
    for original_line in question_lines:
        ...
        for line in segments:
            ...
            if current["section_type"] in (SECTION_RD, SECTION_LS) and len(line) >= 24 and not current["options"]:
