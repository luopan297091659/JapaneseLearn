...
(async () => {
  ...
  try {
    await conn.query('DELETE FROM kana');
    console.log('Cleared existing');
    
    for (const [ch, r, t, cat, o] of kana) {
      const id = uuid();
      await conn.query(
        'INSERT INTO kana SET id=?, `type`=?, `character`=?, romanization=?, category=?, order_index=?, created_at=NOW(), updated_at=NOW()',
        [id, t, ch, r, cat, o]
      );
    }
    
    const [rows] = await conn.query('SELECT COUNT(*) as cnt FROM kana');
    console.log('✅ Imported ' + rows[0].cnt + ' kana records');
    await conn.end();
  } catch(err) {
