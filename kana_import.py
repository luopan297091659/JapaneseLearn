...
kana = [
    ...
    (4,"ぽ","ポ","po",4,6),(5,"きゃ","キャ","kya",0,4),(5,"きゅ","キュ","kyu",1,5),(5,"きょ","キョ","kyo",2,4),
    (5,"しゃ","シャ","sha",3,4),(5,"しゅ","シュ","shu",4,4),(5,"しょ","ショ","sho",5,4),(5,"ちゃ","チャ","cha",6,5),
    (5,"ちゅ","チュ","chu",7,5),(5,"ちょ","チョ","cho",8,5),(5,"にゃ","ニャ","nya",9,3),(5,"にゅ","ニュ","nyu",10,4),
    (5,"にょ","ニョ","nyo",11,3),(5,"ひゃ","ヒャ","hya",12,4),(5,"ひゅ","ヒュ","hyu",13,5),(5,"ひょ","ヒョ","hyo",14,4),
    (5,"みゃ","ミャ","mya",15,6),(5,"みゅ","ミュ","myu",16,7),(5,"みょ","ミョ","myo",17,6),(5,"りゃ","リャ","rya",18,3),
    (5,"りゅ","リュ","ryu",19,4),(5,"りょ","リョ","ryo",20,3),(5,"ぎゃ","ギャ","gya",21,5),(5,"ぎゅ","ギュ","gyu",22,6),
    (5,"ぎょ","ギョ","gyo",23,5),(5,"じゃ","ジャ","ja",24,5),(5,"じゅ","ジュ","ju",25,5),(5,"じょ","ジョ","jo",26,5),
    (5,"びゃ","ビャ","bya",27,5),(5,"びゅ","ビュ","byu",28,6),(5,"びょ","ビョ","byo",29,5),(5,"ぴゃ","ピャ","pya",30,6),
    (5,"ぴゅ","ピュ","pyu",31,7),(5,"ぴょ","ピョ","pyo",32,6),
]

sqls = ["SET FOREIGN_KEY_CHECKS=0;", "DELETE FROM kana_characters;"]
for c, h, k, r, i, s in kana:
    uid = str(uuid.uuid4())
    sqls.append(f"INSERT INTO kana_characters (id, category_id, hiragana, katakana, romaji, order_index, stroke_count) VALUES ('{uid}', {c}, '{h}', '{k}', '{r}', {i}, {s});")
sqls.append("SET FOREIGN_KEY_CHECKS=1;")
sqls.append("SELECT COUNT(*) FROM kana_characters;")

sql_text = "\n".join(sqls)
result = subprocess.run(
    ["mysql", "-u", "root", "-p6586156", "japanese_learn"],
    input=sql_text,
    capture_output=True,
    text=True,
    encoding='utf-8'
...
print(result.stdout if result.returncode == 0 else result.stderr[:1000])
