...
const _kQs = <_Q>[
  ...
  _Q('v', '食べる → 使役受身', '食べさせられる', ['食べさせられる', '食べされる', '食べさされる'],
      '一段：去る+させられる。无短縮形。'),
...
class _TetrisGrammarGameState extends State<TetrisGrammarGame> {
  ...
  Future<void> _fetchConfig() async {
    try {
      ...
      if (res['ok'] == true) {
        setState(() => _maxLevels =
            int.tryParse(res['config']['max_levels'].toString()) ?? 30);
      }
    } catch (_) {}
  }
  ...
  Future<void> _syncServerProgress() async {
    try {
      final res = await ApiService()
          .get('/game/my-progress?game_type=${widget.gameType}');
      ...
      for (final entry in serverScores.entries) {
        final lv = int.tryParse(entry.key);
        if (lv == null) continue;
        final sv = entry.value as Map<String, dynamic>;
        final local = _levelScores[lv];
        if (local == null ||
            (sv['score'] as num? ?? 0) > (local['score'] as num? ?? 0)) {
          _levelScores[lv] = {
            'score': (sv['score'] as num?)?.toInt() ?? 0,
            'stars': (sv['stars'] as num?)?.toInt() ?? 1,
            'combo': (sv['combo'] as num?)?.toInt() ?? 0
          };
        }
      }
      // 移除本地有但服务器没有的旧数据
      _levelScores.removeWhere((lv, _) =>
          !serverScores.containsKey(lv.toString()) && lv < serverUnlocked);
      final p = await SharedPreferences.getInstance();
      await p.setInt(_keyUnlocked, _unlockedTo);
      await p.setString(_keyScores,
          jsonEncode(_levelScores.map((k, v) => MapEntry(k.toString(), v))));
      if (mounted) setState(() {});
  ...
  void _beginLevel(int lv) {
    ...
    _gCols = _lvCols(lv);
    _gRows = _lvRows(lv);
    _livesMax = _lvLives(lv);
    _lives = _livesMax;
    _passTarget = _gRows * _gCols;
    _passCount = 0;
    _score = 0;
    _combo = 0;
    _maxCombo = 0;
    ...
    _boardClr = List.generate(
        _gRows,
        (_) => List.generate(_gCols,
            (_) => <Color>[const Color(0xFFf1f5f9), const Color(0xFFf1f5f9)]));
    _running = true;
    _phase = _Phase.play;
    setState(() {});
    _nextQ();
  ...
  void _nextQ() {
    if (!_running) return;
    if (_board[0].every((c) => c != null)) {
      _running = false;
      Future.delayed(const Duration(milliseconds: 300), _levelFail);
      return;
    }
    final cols = List.generate(_gCols, (i) => i)..shuffle(_random);
    int col = -1;
    for (final c in cols) {
      if (_board[0][c] == null) {
        col = c;
    ...
    final pool = _kQs
        .where((q) =>
            _lvQTypes(_currentLevel, gameType: widget.gameType).contains(q.t))
        .toList();
    // 关内去重：优先选未出过的题
    ...
    final options = List<String>.from(picked.o)..shuffle(_random);
    _curQ = _Q(picked.t, picked.s, picked.a, options, picked.e);
    _usedQuestions.add(picked.s);
    _dropCol = col;
    _selected = _curQ!.o.length ~/ 2;
    _feedbackIdx = null;
    _timeProgress = 1.0;
    _startTimer();
    setState(() {});
  ...
  void _confirm() {
    ...
    int landRow = -1;
    for (int r = _gRows - 1; r >= 0; r--) {
      if (_board[r][_dropCol] == null) {
        landRow = r;
        break;
      }
    }
    ...
    setState(() {
      ...
      if (ok) {
        ...
        _wrongLog.add(
            {'s': _curQ!.s, 'wrong': ans, 'correct': _curQ!.a, 'e': _curQ!.e});
        // 即时显示正确答案解析
  ...
  void _clearRows() {
    for (int r = 0; r < _gRows; r++) {
      if (_board[r].every((c) => c == 'correct')) {
        ...
        _boardClr[0] = List.generate(_gCols,
            (_) => <Color>[const Color(0xFFf1f5f9), const Color(0xFFf1f5f9)]);
  ...
  void _levelClear() {
    ...
    final stars = (acc >= 85 && _maxCombo >= 5)
        ? 3
        : acc >= 65
            ? 2
            : 1;
        ...
        title: Column(children: [
          ...
          Text('★' * stars + '☆' * (3 - stars),
              style: const TextStyle(
                  fontSize: 26, letterSpacing: 3, color: Color(0xFFf59e0b))),
        ]),
        content: SizedBox(
            width: double.maxFinite, child: _reportContent(cleared: true)),
        actions: [
          TextButton(
              onPressed: () {
  ...
  void _levelFail() {
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        title: const Text('💔 关卡失败',
            style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
        content: SizedBox(
            width: double.maxFinite, child: _reportContent(cleared: false)),
        actions: [
          TextButton(
              onPressed: () {
                Navigator.pop(dialogContext);
  ...
  Widget _reportContent({bool cleared = false}) {
    ...
    return Column(
      ...
      children: [
        // 统计行
        Row(children: [
          _statCard('得分', '$_score', const Color(0xFFe8effe),
          ...
          const SizedBox(width: 8),
          _statCard('最高连击', '$_maxCombo', const Color(0xFFfffbeb),
              const Color(0xFFf59e0b)),
        ]),
        if (!cleared) ...[
          const SizedBox(height: 8),
          Text('已答对 $_passCount/$_passTarget，未达过关条件',
        ...
        if (_allLog.isNotEmpty) ...[
          ...
          Align(
            ...
            child: Text('📋 本关全部题目（${_allLog.length}题）',
                style:
              ...
              itemBuilder: (_, i) {
                ...
                return Container(
                  ...
                  decoration: BoxDecoration(
                    color:
                        ok ? const Color(0xFFf0fdf4) : const Color(0xFFfef2f2),
                    border: Border(
                        left: BorderSide(
                            color: ok
                    ...
                    children: [
                      ...
                      if (w['t'] == 'p' && w['o'] != null)
                        ...(() {
                          ...
                          final others =
                              options.where((o) => o != w['correct']).toList();
                          if (others.isEmpty) return <Widget>[];
                          return <Widget>[
                            const SizedBox(height: 2),
                            Text(
                                '其他选项：${others.map((o) => _optionExplain(o, 'p')).join('；')}',
                                style: const TextStyle(
  ...
  Future<void> _submitScore() async {
    try {
      ...
      apiService.logActivity(
          activityType: 'grammar', durationSeconds: 0, score: acc.toDouble());
    } catch (_) {}
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
          child: _phase == _Phase.select ? _buildSelect() : _buildPlay()),
    );
  }
  ...
  Future<void> _showLeaderboard() async {
    showDialog(
      ...
      builder: (dialogCtx) => FutureBuilder<Response<dynamic>>(
        future: ApiService().dio.get('/game/leaderboard/global',
            queryParameters: {'game_type': widget.gameType}),
        builder: (ctx, snap) {
          ...
          return AlertDialog(
            ...
            content: SizedBox(
              ...
              child: rows.isEmpty
                  ? const Center(
                      child: Padding(
                          padding: EdgeInsets.all(20), child: Text('暂无排行数据')))
                  : ListView.separated(
                      shrinkWrap: true,
                        ...
                        return ListTile(
                          ...
                          title: Text(
                            ...
                            style: TextStyle(
                                ...
                                color: isSelf ? const Color(0xFF4361ee) : null),
                          ),
                          subtitle: Text(
                              '最高关 ${r['max_level'] ?? 0} · 连击 ${r['best_combo'] ?? 0}'),
    ...
    return Column(
      children: [
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                  child: Text('关卡 $_currentLevel · $_gRows×$_gCols',
              ...
              _livesMax >= 99
                  ...
                  : Text(
                      '❤️' * max(0, _lives) + '🖤' * max(0, _livesMax - _lives),
            ...
            children: [
              Expanded(
                ...
                child: LinearProgressIndicator(
                  value: _passTarget > 0 ? _passCount / _passTarget : 0,
                  minHeight: 6,
        ...
        Padding(
          ...
          child: Row(
            children: [
              Text('${_speedOptions[_speedIdx]['icon']}',
                  style: const TextStyle(fontSize: 14)),
              const SizedBox(width: 4),
              Text('速度 ${_speedOptions[_speedIdx]['label']}',
                  style: const TextStyle(
                      fontSize: 12,
                ...
                children: [
                  ...
                  ClipRRect(
                    ...
                    child: LinearProgressIndicator(
                      ...
                      minHeight: 5,
                      backgroundColor: const Color(0xFFe2e8f0),
                      valueColor: AlwaysStoppedAnimation(_timeProgress > 0.5
                          ? const Color(0xFF4361ee)
                          : _timeProgress > 0.25
                              ? const Color(0xFFf59e0b)
          ...
          Padding(
            ...
            child: Row(
              children: List.generate(q.o.length, (i) {
                ...
                Color bdr =
                    isSel ? const Color(0xFF4361ee) : const Color(0xFFe2e8f0);
                Color txt =
                    isSel ? const Color(0xFF4361ee) : const Color(0xFF64748b);
                if (isFb) {
                  bg = _feedbackOk
                      ? const Color(0xFFdcfce7)
                ...
                if (_feedbackIdx != null && !_feedbackOk && q.o[i] == q.a) {
                  bg = const Color(0xFFdcfce7);
                  bdr = const Color(0xFF16a34a);
                  txt = const Color(0xFF15803d);
                }
                final maxLen =
                    q.o.fold(0, (m, s) => s.length > m ? s.length : m);
                final fs = maxLen > 4
                    ? 12.0
                    : maxLen > 2
                        ? 14.0
              ...
              children: List.generate(
                  ...
                  (r) => Row(
                        children: List.generate(_gCols, (c) {
                          final st = _board[r][c];
                          final txt = _boardTxt[r][c];
                          final clr = _boardClr[r][c];
